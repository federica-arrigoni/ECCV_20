...
%% TO DO: PERMUTATION SYNCHRONIZATION 
%%
function [group]=segment_mode_triplets(labels_triplets,triplets,tracks,dim,ncams,d,method_permutation)
...
Q_pair=sparse(d*ntriplets,d*ntriplets);


for k=1:ntriplets
    tr_k=triplets{k};
    groups_k=labels_triplets{k}; % labels of triplet k
    ...
    for h=k+1:ntriplets
        ...
        tr_h=triplets{h};
        groups_h=labels_triplets{h}; % labels of triplet k
        ...
        if ~isempty(images)
            for l=1:length(images)
                ...
                labels_k=labels_full(tracks,triplets,dim,images(l),k,groups_k);
                labels_h=labels_full(tracks,triplets,dim,images(l),h,groups_h);
...
Q_synch = pachauri_synch(Q_pair,d,ntriplets,method_permutation);
%lambda=eig(Q_pair);
...
for k=1:ntriplets
    ...
    % Go back to labels
    labels_triplets{k}=matrix2labels(P_current);
...
triplets_matrix=cell2mat(triplets); % DA CAMBIARE SE VOGLIAMO ESTENDERE A SOTTOGRAFI DI DIMENSIONI DIVERSE INVECE CHE TRIPLE
for i=1:ncams
    
    % find triplets involving image i 
    index_i=find(sum(triplets_matrix==i,2)>0);
    ...
    for k=1:n_estimates
        ...
        groups_tr=labels_triplets{tr}; % labels of current triplet
        
        % transform labels of the tracks in the triplet into labels of
        % points in image i
        [current_estimate]=labels_full(tracks,triplets,dim,i,tr,groups_tr);
